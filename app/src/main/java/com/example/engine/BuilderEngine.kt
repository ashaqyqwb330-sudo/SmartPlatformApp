...
class BuilderEngine(
    ...
    fun getBaseDirForPrefix(prefix: String): File {
        ...
        val defPath = settings["base_dir"] as? String ?: context.filesDir.absolutePath
        val baseDir = File(defPath)
        baseDir.mkdirs()
        return baseDir
    }
    ...
    suspend fun writeFile(
        ...
        try {
            ...
            if (mode == "a") {
                full.appendText(content, Charsets.UTF_8)
            } else {
    ...
    suspend fun processText(text: String): List<ProcessResult> = withContext(Dispatchers.IO) {
        ...
        if (hasBuilder) {
            ...
            for ((i, blk) in blocks.withIndex()) {
                ...
                for ((path, content, mode, baseDir) in processBlock(blk, text, start, bPrefixes)) {
                    val res = writeFile(path, content, mode, baseDir)
                    res.onSuccess { fwr ->
                        results.add(ProcessResult("builder", fwr.message, mapOf(
                            "path" to path, "size" to fwr.size.toString(), "mode" to fwr.mode, "content" to content, "fullPath" to fwr.fullPath
                        )))
                    }.onFailure { e ->
        ...
        if (hasExecutor) {
            ...
            for (line in lines) {
                ...
                val resMsg = executeDirective(cmd)
     ...
     */
    suspend fun executeDirective(command: String): String = withContext(Dispatchers.IO) {
        try {
            when {
                command == "build" -> {
                ...
                command.startsWith("run ") -> {
                    ...
                    val process = Runtime.getRuntime().exec(arrayOf("sh", "-c", "echo 'Simulating script execution $script'"))
                    process.waitFor()
                    "✅ [EXEC] تم تشغيل $script (محاكاة)"
                }
                command.startsWith("open ") -> {
                    val target = command.removePrefix("open ").trim()
                    val baseDir = File(settings["base_dir"] as? String ?: context.filesDir.absolutePath)
                    val file = File(baseDir, target)
                    if (file.exists()) {
                        "✅ [EXEC] ملف مفتوح في الواجهة: $target"
                    } else {
                        "⚠️ [EXEC] المسار غير موجود: $target"
                    }
                }
                else -> {
                    "✅ [EXEC] تم تنفيذ الأمر: $command"
                }
            }
        } catch (e: Exception) {
            "❌ [EXEC] فشل: ${e.message}"
    ...
    suspend fun runTreedoc(folder: String, fmt: String, copyClipboard: Boolean): Pair<String, Map<String, String>?> = withContext(Dispatchers.IO) {
        try {
            val baseDir = File(settings["base_dir"] as? String ?: context.filesDir.absolutePath)
            val target = if (folder == "." || folder.isEmpty()) {
                baseDir
            } else {
                File(folder).let {
                    if (it.isAbsolute && it.exists()) it else File(baseDir, folder)
                }
            }
            ...
            if (tree.isEmpty()) return@withContext "⚠️ [TREEDOC] المجلد فارغ: $folder" to null
            ...
            if (copyClipboard) {
                ...
                withContext(Dispatchers.Main) {
                    try {
                        ...
                        Log.e(TAG, "Cannot write clipboard on background or without UI context", e)
                    }
                }
            }

            // حفظ الملف
            val outFile = File(target, "tree_report.$fmt")
    ...
    class TreeNode(
        val name: String,
        val path: String,
        val type: String, // "directory" or "file" or "error"
    ...
    fun collectTreeData(
        ...
        val node = TreeNode(
            ...
            if (showMtime) {
                mtime = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date(directory.lastModified()))
            }
        }

        try {
            val entries = directory.listFiles()?.filter { file ->
                file.name !in IGNORE_DIRS && exclude.none { ex -> ex in file.name }
            ...
            for (entry in entries) {
                if (entry.isDirectory) {
                    ...
                    val fileNode = TreeNode(
                        ...
                        if (showSize) size = formatSize(entry.length())
                        if (showMtime) mtime = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date(entry.lastModified()))
                    }
    ...
    private fun buildTextLines(nodes: List<TreeNode>, prefix: String, isLast: Boolean, showSize: Boolean, showMtime: Boolean, showCount: Boolean, sb: StringBuilder) {
        for ((i, node) in nodes.withIndex()) {
            ...
            if (showCount && node.count != null && node.type == "directory") extras.add("📁 ${node.count} عنصر")
    ...
    private fun buildJsonReport(tree: List<TreeNode>): String {
        // تقرير JSON سريع ومبسط يدوي لتفادي الإعتماد على مكتبات خارجية قد تسبب مشاكل توافقية
        val sb = StringBuilder()
        sb.append("{\n")
        ...
        for ((i, node) in tree.withIndex()) {
            ...
            sb.append("      \"path\": \"${node.path.replace("\\", "\\\\")}\",\n")
            sb.append("      \"items_count\": ${node.count ?: 0}\n")
            sb.append("    }${if (i < tree.size - 1) "," else ""}\n")
        }
        sb.append("  ]\n")
        sb.append("}")
        return sb.toString()
